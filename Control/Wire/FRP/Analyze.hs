...
avg ::
    (Applicative m, Fractional a, Fractional t, HasTime t ds, Monoid e, Real t)
    => t    -- ^ Interval size.
...
avgFps ::
    (Applicative m, Fractional b, Fractional t, HasTime t ds, Monad m, Monoid e, Real t)
    => t    -- ^ Interval size.
    -> Int  -- ^ Number of data points to use.
    -> Wire ds e m a b
...
framerate ::
    (Applicative m, Fractional b, HasTime t ds, Monoid e, Real t)
    => Wire ds e m a b
framerate =
    mkPure_ $ \ds _ ->
        let dt = dtime ds in
...
highPeak :: (Applicative m, Ord a) => Wire ds e m a a
...
highPeakBy :: (Applicative m) => (a -> a -> Ordering) -> Wire ds e m a a
...
lowPeak :: (Applicative m, Ord a) => Wire ds e m a a
...
lowPeakBy :: (Applicative m) => (a -> a -> Ordering) -> Wire ds e m a a
lowPeakBy = peakBy LT
